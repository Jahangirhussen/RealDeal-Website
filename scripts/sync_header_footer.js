...
const HEADER_SRC_RAW = fs.readFileSync(path.join(ROOT, 'header.html'), 'utf-8');
const HEADER_SCRIPT_MATCH = HEADER_SRC_RAW.match(WHATSAPP_SCRIPT_RE);
const HEADER_SCRIPT_LINE = HEADER_SCRIPT_MATCH ? HEADER_SCRIPT_MATCH[0] : '';
const HEADER_SRC = HEADER_SRC_RAW.replace(WHATSAPP_SCRIPT_RE, '');
...
const NAV_LINKS = {
  home: ['a', 'index.html', 'Home'],
  services: ['dropdown', 'services.html', null],
  packages: ['a', 'packages.html', 'Packages'],
  portfolio: ['a', '__PORTFOLIO__', 'Portfolio'],
  blog: ['a', '__BLOG__', 'Blog'],
  about: ['a', 'about.html', 'About us'],
  career: ['a', 'careers.html', 'Career'],
  contact: ['a', 'contact.html', 'Contact us'],
};
...
function rewrite(html, depth, folder) {
  ...
  html = html.replace(/(href|src)="([a-zA-Z0-9\-]+\.html)((?:[#?][^"]*)?)"/g, (m, attr, fname, frag) => {
...
function bakeInto(fullPath, depth, folder) {
  ...
  const scriptLine = HEADER_SCRIPT_LINE ? rewrite(HEADER_SCRIPT_LINE, depth, folder) : '';
  content = content.replace(HEADER_BLOCK_RE, () => scriptLine + newHeader);
